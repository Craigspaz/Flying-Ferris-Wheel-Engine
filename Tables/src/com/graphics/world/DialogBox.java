...
public class DialogBox
{
	...
	private int			currentLetter;								// the current index of a letter in the current
																	// string
	private int			currentLine;								// a number 0-4 that represents the current line on
																	// a page
	private String		currentString;								// the temporary string for the current line being
																	// typed
	...
	public void render(float textBoxX, float textBoxY)
	{
		if (isActive)
		{
			...
			GFX.drawSpriteFromSpriteSheet(1024, 256, textBoxX, textBoxY, textBoxes, new Vector2f(0, textBoxNumber), new Vector2f(1, 1), -1, 1f);// TODO
																																				// fix
																																				// the
																																				// vectors
																																				// so
			...
			GFX.drawString2(textBoxX + 138, textBoxY + 30 + (currentLine * lineheight), currentString);// draws the
																										// typed string
		}
	}
	...
	public void update(InputHandler handler)
	{
		if (handler == null)
		{
			throw new NullPointerException("The dialog box can't do anything without input handling");
		}
		if (animateTime >= animateFrameTime)// for animating text appearance
		{
			...
			} else
			{
				...
				if (currentLine + (pageNumber * 5) < messages.length - 1 && currentLine < 4)// increments the line
																							// number and resets line
																							// letter when at the end of
																							// a line (unless on the
																							// last line of a page)
				{
					currentLetter = 0;
					currentLine++;
			...
			currentString = messages[currentLine + (pageNumber * 5)].substring(0, currentLetter);// takes a substring
																									// from the start to
																									// the current
																									// letter of the
																									// current line
		...
		if (handler.nextPage() && canPress)// waits for user input when at the end of a page, or at the end of all
											// messages
		{
			canPress = false;
			if (currentLine + (pageNumber * 5) == messages.length - 1 && currentLetter == messages[currentLine + (pageNumber * 5)].length())// if
																																			// it's
																																			// at
																																			// the
																																			// end
			...
			{
				isActive = false;
			} else if (currentLine == 4 && currentLetter == messages[currentLine + (pageNumber * 5)].length())// if it's
																												// on
																												// the
																												// last
																												// line
