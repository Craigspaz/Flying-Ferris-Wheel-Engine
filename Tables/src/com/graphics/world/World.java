		...
		try
		{
			...
			while (scanner.hasNextLine())
			{
				...
				} else if (line.trim().startsWith("<TILE "))
				{
					...
					int texCoordX = Math.round(Float.parseFloat(tCoordX));
					int texCoordY = Math.round(Float.parseFloat(tCoordY));
					tiles.add(new Tile(new Vector3f(xPos, yPos, zPos), new Vector2f(64, 64), Textures.tilesheet, 1, 0));
